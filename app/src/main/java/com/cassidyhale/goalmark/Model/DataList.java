...
public class DataList extends Application implements Parcelable { //
    ...
    public boolean needsUpdateToFire;
    ...
    public DataList (String userId, long dateLastUpdate, List<Goal> listOngoing, List<Goal> listCompleted, Schedule schedule, boolean needsUpdate) {
        this.userId = userId;
    ...
    public void setNeedsUpdateToFire(boolean needsUpdateToFire) {
        this.needsUpdateToFire = needsUpdateToFire;
    }
    ...
    public void displayAll ()  {
        StringBuilder liOnCom = new StringBuilder();
        liOnCom.append("\n");
        liOnCom.append("\n");
        liOnCom.append("     DATALIST = \n");
        Date date = Calendar.getInstance().getTime();
        date.setTime( this.dateLastUpdate);
        liOnCom.append("        Date (last update): "+ date.toString() );
        liOnCom.append("     >>> Goals ONGOING: \n");
        for (int i=0; i < this.getListOngoing().size(); i++)    {
            liOnCom.append("\n");
            liOnCom.append("            Goal "+ i +" :      "+ this.getListOngoing().get(i).getgLabel() +" :\n");
            liOnCom.append("             List Sungoal:\n");
            for (int j=0; j < this.getListOngoing().get(i).getgListSubGoals().size(); j++)    {
                liOnCom.append("                Subgoal "+ j +" :      "+ this.getListOngoing().get(i).getgListSubGoals().get(j).getsLabel() +" :\n");
                for (int k=0; k < this.getListOngoing().get(i).getgListSubGoals().get(j).getsListTasks().size(); k++)    {
                    liOnCom.append("                      Task "+ k +" :      "+ this.getListOngoing().get(i).getgListSubGoals().get(j).getsListTasks().get(k).getTaskLabel() +" :\n");
                }
            }
            liOnCom.append("                List Tasks:\n");
            for (int j=0; j < this.getListOngoing().get(i).getgListTasks().size(); j++)    {
                liOnCom.append("                Task "+ j +" :      "+ this.getListOngoing().get(i).getgListTasks().get(j).getTaskLabel() +" :\n");
            }
        }
        liOnCom.append("\n");
        liOnCom.append("\n");
        Log.d("Userx", liOnCom.toString());
    }
    ...
    public void displayDateLastUpdated  () {
        Date date = Calendar.getInstance().getTime();
        date.setTime( this.dateLastUpdate);
        Log.d(__GLOBAL_Const.TAG_handleUserfb,"        Date (last update): "+ date.toString() );
    }
    ...
    public static DataList fromMap (Map<String, Object> map )   {
        DataList createdDataList = new DataList(
    ...
    private void createNotificationChannels()   {
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.O){
            CharSequence name = getResources().getString(R.string.stringNotificationChannelName);
            String description = getResources().getString(R.string.stringNotificationChannelIdDescript);

            NotificationChannel channel1 = new NotificationChannel(
                    CHANNEl_ID_1,
                    "Channel 1",
                    NotificationManager.IMPORTANCE_HIGH);
            channel1.setDescription(" My Channel Description");
